...
export function load({ url }) {
  ...
  // Load this scouter's shift assignments
  const schedule = db.prepare(`
    ...
    WHERE sa.scouter_id = ?
    ORDER BY m.match_number
  `).all((scouter as any).id) as any[];

  // Already scouted match IDs by this scouter
  ...
  return { scouter, matches, teams, schedule, scoutedMatchIds };
}
