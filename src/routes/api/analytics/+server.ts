...
export function GET({ url }) {
  ...
  if (team) {
    const teamData = db.prepare('SELECT * FROM teams WHERE number = ?').get(Number(team));
    const matchData = db.prepare(`
      ...
      WHERE team_number = ?
    `).get(Number(team));
    const prescout = db.prepare('SELECT * FROM prescout_responses WHERE team_number = ? ORDER BY created_at DESC LIMIT 1').get(Number(team));

    return json({ team: teamData, stats: matchData, prescout });
  }
