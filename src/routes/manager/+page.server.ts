...
export function load() {
  ...
  const matchScoutCount = (db.prepare('SELECT COUNT(*) as c FROM match_scouts').get() as any).c;
  const prescoutCount = (db.prepare('SELECT COUNT(*) as c FROM prescout_responses').get() as any).c;
