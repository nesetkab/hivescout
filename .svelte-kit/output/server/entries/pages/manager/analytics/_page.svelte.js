...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		if (data.ranked.length === 0) {
			...
			$$renderer.push("<!--[-1-->");
			$$renderer.push(`<div class="table-wrap svelte-138aw5j"><table class="svelte-138aw5j"><thead><tr><th class="svelte-138aw5j">#</th><th class="svelte-138aw5j">Team${escape_html(arrow("number"))}</th><th class="svelte-138aw5j">M${escape_html(arrow("matches"))}</th><th class="svelte-138aw5j">Scored${escape_html(arrow("avgScored"))}</th><th class="svelte-138aw5j">Att${escape_html(arrow("avgAttempted"))}</th><th class="svelte-138aw5j">Acc${escape_html(arrow("accuracy"))}</th><th class="svelte-138aw5j">Leave${escape_html(arrow("autoLeaveRate"))}</th><th class="svelte-138aw5j">Park${escape_html(arrow("parkRate"))}</th><th class="svelte-138aw5j">Gate${escape_html(arrow("gateRate"))}</th><th class="svelte-138aw5j">Skill${escape_html(arrow("avgSkill"))}</th><th class="svelte-138aw5j">Rel${escape_html(arrow("avgReliability"))}</th><th class="svelte-138aw5j">Score${escape_html(arrow("totalScore"))}</th></tr></thead><tbody><!--[-->`);
			const each_array_1 = ensure_array_like(filtered());
			for (let $$index_1 = 0, $$length = each_array_1.length; $$index_1 < $$length; $$index_1++) {
				let team = each_array_1[$$index_1];
				$$renderer.push(`<tr><td${attr_class(`rank-cell ${stringify(tierColor(team.rank, data.ranked.length))}`, "svelte-138aw5j")}>${escape_html(team.rank)}</td><td class="team-num svelte-138aw5j"><a${attr("href", `/manager/analytics/${stringify(team.number)}`)} class="svelte-138aw5j">#${escape_html(team.number)}</a> <span class="table-name svelte-138aw5j">${escape_html(team.name)}</span></td><td class="svelte-138aw5j">${escape_html(team.matches)}</td><td class="svelte-138aw5j">${escape_html(team.avgScored)}</td><td class="svelte-138aw5j">${escape_html(team.avgAttempted)}</td><td class="svelte-138aw5j">${escape_html(team.accuracy)}%</td><td class="svelte-138aw5j">${escape_html(team.autoLeaveRate)}%</td><td class="svelte-138aw5j">${escape_html(team.parkRate)}%</td><td class="svelte-138aw5j">${escape_html(team.gateRate)}%</td><td class="svelte-138aw5j">${escape_html(team.avgSkill)}</td><td class="svelte-138aw5j">${escape_html(team.avgReliability)}</td><td class="score-cell svelte-138aw5j">${escape_html(team.totalScore)}</td></tr>`);
			}
